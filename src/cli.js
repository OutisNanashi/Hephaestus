...
function runInternal(argv, handlers = {}) {
  ...
  if (!["status","dashboard","validate","inspect","live-brain","cycle","sandbox-run","agent-run","agent-preflight","agent-run-plan","agent-smoke","agent-discover","agent-codex-readonly-smoke","agent-codex-readonly-inspect","verify-tests","git-branch","git-commit","pr-open"].includes(command) && !reviewIngest && !mergeCommand && !notifyRender) throw new HephaestusError(`Unknown command: ${command}.`, "INVALID_ARGUMENT");

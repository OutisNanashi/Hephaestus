...
import { AUTO_MERGE_MODE, MANUAL_MERGE_MODE } from "./auto-merge.js";
...
function parseArguments(argv) {
  ...
  while (args.length > 0) {
    ...
    else if (option === "--providers") providersFlag = true;
    else if (option === "--preflight") preflightFlag = true;
  ...
  return { command, mergeSubcommand, mergeProjectId, notifySubcommand, notifyProjectId, configPath, projectId, saveReport, mockGptPath, mockAgentOutputPath, commandId, adapterId, promptPath, task, message, provider, fixturePath, maxCycles, autoMerge, manualMerge, providersFlag, preflightFlag };
}
...
export const KNOWN_COMMANDS = Object.freeze([
  "status", "dashboard", "validate", "inspect", "live-brain", "run-live", "next-phase", "cycle", "sandbox-run",
...
function runInternal(argv, handlers = {}) {
  const { command, mergeSubcommand, mergeProjectId, notifySubcommand, notifyProjectId, configPath, projectId, saveReport, mockGptPath, mockAgentOutputPath, commandId, adapterId, promptPath, task, message, provider, fixturePath, maxCycles, autoMerge, manualMerge, providersFlag, preflightFlag } = parseArguments(argv);
  ...
  if (providersFlag && command !== "status") throw new HephaestusError("--providers only applies to status.", "INVALID_ARGUMENT");
  if (preflightFlag && !(command === "status" && providersFlag)) throw new HephaestusError("--preflight only applies to status --providers.", "INVALID_ARGUMENT");
  ...
  if (command === "next-phase") {
    if (typeof handlers.nextPhase !== "function") throw new HephaestusError("next-phase requires the async CLI runner.", "INVALID_ARGUMENT");
    return handlers.nextPhase({ config, project });
  }
...
export async function runAsync(argv) {
  return runInternal(argv, {
    ...
    liveLoop: async ({ config, project, task, maxCycles, mergeMode }) => {
      const result = await orchestrateRunLive({ mode: mergeMode, config, project, task, maxCycles });
      if (result.mode === MANUAL_MERGE_MODE) {
        process.stdout.write(`${JSON.stringify({ mode: result.mode, autoMerge: false, ...loopOutput(result.loop), merge: result.merge ?? null }, null, 2)}\n`);
        return result.loop.status === "task-complete" ? 0 : 1;
      }
      const merged = result.phases.filter((phase) => phase.stage === "merged").length;
      process.stdout.write(`${JSON.stringify({ mode: result.mode, autoMerge: true, outcome: result.outcome, reason: result.reason ?? null, phasesMerged: merged, phases: result.phases }, null, 2)}\n`);
      return result.outcome === "project-complete" || merged > 0 ? 0 : 1;
    },
    nextPhase: async ({ config, project }) => {
      ...
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return result.status === "phase-ready" ? 0 : result.status === "all-complete" ? 0 : 1;
    },

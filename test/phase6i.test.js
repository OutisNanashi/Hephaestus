...
test("CLI agent-codex-readonly-prompt-record returns non-zero when codex is unavailable on PATH", () => {
  ...
  try {
    ...
    try {
      ...
      exitCode = runCli(["agent-codex-readonly-prompt-record", "--config", configPath, "--project", "example-project"]);
...
test("spawned CLI agent-codex-readonly-prompt-record exits non-zero when codex is missing", () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [CLI_PATH, "agent-codex-readonly-prompt-record", "--config", configPath, "--project", "example-project"], {
      encoding: "utf8",
      shell: false,
      env,
      timeout: 60_000

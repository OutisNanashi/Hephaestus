...
test("CLI agent-codex-readonly-inspect-record returns non-zero when codex is unavailable on PATH", () => {
  ...
  try {
    ...
    try {
      ...
      exitCode = withEmptyPath(emptyPathDir, () => runCli(["agent-codex-readonly-inspect-record", "--config", configPath, "--project", "example-project"]));
...
test("spawned CLI agent-codex-readonly-inspect-record exits non-zero when codex is missing", () => {
  ...
  try {
    ...
    const result = spawnCliSync(process.execPath, [CLI_PATH, "agent-codex-readonly-inspect-record", "--config", configPath, "--project", "example-project"], {
      encoding: "utf8",
      shell: false,
      env,
      timeout: 60_000

...
function repo() { const d=fs.mkdtempSync("/tmp/hephaestus-git-"); git(d,"init"); git(d,"config","user.name","Fixture"); git(d,"config","user.email","fixture@local"); fs.writeFileSync(path.join(d,"a.txt"),"a\n"); git(d,"add","a.txt"); git(d,"commit","-m","initial"); return d; }
